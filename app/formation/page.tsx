...
// ================================================
// MODIFICATION ICI : Ajout de 'useEffect'
// ================================================
import React, { useRef, useState, useEffect } from 'react';
import Image from 'next/image';
          ...
          {theme.modules.map((module, index) => (
            <div
              ...
              onClick={() => {
                ...
                setSelectedModule(module);
                // =======================================================
                // CORRECTION : Doit être 'formation-detail'
                // =======================================================
            ...
            >
              ...
              >
                <div
                  className={`relative rounded-md overflow-hidden transition-opacity duration-300 ${
                    hoveredModule === module.code
                      ? 'opacity-0'
                      : 'opacity-100'
                  }`}
...
export default function FormationPage({
  ...
  // =================================================================
  // NOUVEAU : Effet pour gérer le défilement par ancre (hash)
  // =================================================================
  useEffect(() => {
    ...
    const scrollToHash = () => {
      ...
      if (hash) {
        // On retire le '#' pour obtenir l'ID
        const elementId = hash.substring(1);
        const element = document.getElementById(elementId);

        if (element) {
          // On attend un court instant que la page soit bien dessinée
          setTimeout(() => {
            element.scrollIntoView({ behavior: 'smooth', block: 'start' });
            // Optionnel : nettoyer le hash de l'URL après le scroll
            // history.replaceState(null, '', ' ');
          }, 150); // 150ms pour plus de sécurité
        }
      }
    };

    // On exécute au montage
    scrollToHash();

    // On écoute aussi les changements de hash (si l'utilisateur clique sur un lien d'ancre DÉJÀ sur la page)
    ...
    <>
      ...
      <section className="pt-8 md:pt-12 pb-4 md:pb-8 bg-white">
        <div className="max-w-[1920px] mx-auto">
          {FORMATION_CATALOGUE.map((theme) => (
            // ================================================
            // MODIFICATION ICI : Ajout du div wrapper avec ID
            // ================================================
            <div
              id={`theme-${theme.slug}`}
              key={theme.slug}
              // h-24 (navbar) = 96px. 120px donne une marge de sécurité
