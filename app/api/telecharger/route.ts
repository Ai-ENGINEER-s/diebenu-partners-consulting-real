...
function analyzeSpam(data: SpamData): SpamCheckResult {
  ...
  // 2. Nom
  if (isGibberish(data.fullName)) { spamScore += 70; reasons.push('Name appears gibberish'); }
  const nameWords = data.fullName.trim().split(/\s+/);
  if (nameWords.length === 1 && data.fullName.length > 8) { spamScore += 40; reasons.push('Single-word name'); }
  if (/[a-z][A-Z]/.test(data.fullName) && data.fullName.length > 10) { spamScore += 35; reasons.push('Random case in name'); }
  if (/\d{3,}/.test(data.fullName)) { spamScore += 30; reasons.push('Name has too many numbers'); }
  if (/[^a-zA-ZÀ-ÿ\s'-]/.test(data.fullName)) { spamScore += 25; reasons.push('Invalid characters in name'); }

  // 3. Email
  const emailDomain = data.email.split('@')[1]?.toLowerCase() || '';
  if (SUSPICIOUS_EMAIL_DOMAINS.some(domain => emailDomain.includes(domain))) { spamScore += 50; reasons.push('Disposable email'); }
  if (/[^a-zA-Z0-9@.\-_+]/.test(data.email)) { spamScore += 30; reasons.push('Invalid characters in email'); }

  // 4. Temps de soumission
  if (data.submissionTime && data.submissionTime < 3000) { spamScore += 40; reasons.push('Submitted too quickly'); }

  // 5. Nom / Email cohérence
  const nameParts = data.fullName.toLowerCase().split(/\s+/);
  const emailUsername = data.email.split('@')[0].toLowerCase();
  const hasNameInEmail = nameParts.some(part => part.length > 2 && emailUsername.includes(part));
  if (!hasNameInEmail && !isGibberish(emailUsername)) { spamScore += 10; reasons.push('Name-email mismatch'); }

  const isSpam = spamScore >= 40;
  return { isSpam, reason: reasons.join(' | '), score: Math.min(spamScore, 100) };
}
 ...
 */
function validateInput(data: any): { valid: boolean; error?: string } {
  if (!data.fullName || !data.email) return { valid: false, error: 'Nom et Email requis' };
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(data.email)) return { valid: false, error: 'Email invalide' };
  if (data.fullName.trim().length < 2 || data.fullName.trim().length > 100) return { valid: false, error: 'Nom invalide (2-100 caractères)' };
  return { valid: true };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Anti-spam
    const spamCheck = analyzeSpam({ fullName, email, honeypot, submissionTime });
...
<div style="font-family: Arial, 'Helvetica Neue', Helvetica, sans-serif; font-size: 16px; line-height: 1.5; color: #333; max-width: 600px; margin: 20px auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
  ...
  <p style="font-size: 12px; color: #777; text-align: center;">&copy; ${currentYear} DIEBENU & PARTNERS. Tous droits réservés.</p>
    ...
    return NextResponse.json({ success: true, message: 'Coordonnées reçues et confirmation envoyée' });
  } catch (error: any) {
    console.error('❌ Erreur:', error);
